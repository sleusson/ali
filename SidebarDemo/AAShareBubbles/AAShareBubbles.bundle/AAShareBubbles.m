...
{
    self = [super initWithFrame:CGRectMake(point.x - radiusValue, point.y - radiusValue, 2 * radiusValue, 5 * radiusValue)];
    if (self) {
        self.radius = radiusValue;
        self.bubbleRadius = 45;
...
-(void)show
{
    if(!self.isAnimating)
    {
        ...
        if(self.showFacebookBubble)     [self createButtonWithIcon:@"AL.png" backgroundColor:self.facebookBackgroundColorRGB andType:AAShareBubbleTypeFacebook];
        if(self.showTwitterBubble)      [self createButtonWithIcon:@"AG.png" backgroundColor:self.twitterBackgroundColorRGB andType:AAShareBubbleTypeTwitter];
        if(self.showGooglePlusBubble)   [self createButtonWithIcon:@"MD.png" backgroundColor:self.googlePlusBackgroundColorRGB andType:AAShareBubbleTypeGooglePlus];
